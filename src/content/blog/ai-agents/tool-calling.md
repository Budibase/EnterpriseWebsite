...
## Tool calling with Budibase Agents

Budibase is the open-source AI workflow toolkit for privacy-focused teams. Connect your tools and data to LLMs, including locally-hosted models, across Budibase Apps, Agents, and Automations.

Budibase Agents are model agnostic, meaning that we can deploy AI within real-world worklfows, without handing our data over to opaque public models and services. We also offer optional self-hosting, free SSO, custom RBAC, and other key capabilities for security conscious teams.

Agent behavior, including tool usage, is configured using natural language prompts. This removes the boilerplate from building agents, offering a flexible, maintainable solution for deploying AI within real-world workflows.

![Budibase tool calling](https://res.cloudinary.com/daog6scxm/image/upload/v1770825652/cms/agents/Group_1_fafmcn.webp "Tool Calling Budibase")

We can connect Agents to any data sources, integrated tools, APIs, or automation rules within our Budibase Workspace. However, Agents can only use the tools and data we explitly grant them access to, meaning we can determine the exact right permissions each agent needs for the task at hand.
...
Budibase is the open-source, AI workflow toolkit for privacy-focused teams.

We offer extensive connectivity for LLMs, RDBMSs, NoSQL tools, APIs, and more, alongside AI agents, autogenerated UIs and an advanced, visual automation builder.
