...
We've also recently launched Budibase Agents with full support for any LLM with an OpenAI-compatible API, including local and open-source models. We can grant our Agents access to a huge range of tools and data sources, and build intelligent assistants, without handing over our data to opaque public models and services.

There’s never been an easier way to build AI workflow securely. Take a look at our [features overview](https://budibase.com/product/) to learn more.