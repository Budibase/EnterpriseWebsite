...
<figure>
  <img
    src={githubBindingsScreenshot}
    alt="GitHub git/get-tree request configuration showing default owner, repo, tree_sha, and recursive bindings in the Budibase API builder."
  />
  <figcaption>The GitHub template uses fixed defaults for repository lookup, with only the selected file path changing later in the flow.</figcaption>
</figure>
...
<figure>
  <img
    src={notionTransformerScreenshot}
    alt="Budibase Notion Block_getChildren API screen showing a JavaScript transformer that extracts headings and paragraph bodies into a compact JSON structure."
  />
  <figcaption>The Notion transformer trims the payload down to the headings and body text that the agent actually needs.</figcaption>
</figure>
...
The workflow requires one BudibaseDB table named `Notion Pages`. The agent queries this table through `budibase.Notion Pages.list_rows` to map a human-readable page name to the Notion page ID used by the API layer.

Store two text fields:

- `name`: the document name the agent can reason about
- `notion_id`: the actual Notion page ID required by the API requests

You can seed the table with a simple CSV:
...
<figure>
  <img
    src={notionPagesTableScreenshot}
    alt="Budibase data table named Notion Pages with rows for HR Policy, Onboarding FAQs, and Operations Manual, each mapped to a notion_id value."
  />
  <figcaption>The `Notion Pages` table acts as the lookup layer between human-readable page names and the Notion IDs required by the API requests.</figcaption>
</figure>

<h2 id="invoking-the-agent">Invoking the agent</h2>

Because this is a conversational assistant, you have several deployment options depending on where employees already work.

One option is to invoke the agent through Budibase Automations. For example, you can pair an `Email Received` trigger with `Send Email (SMTP)` so the assistant can monitor a shared support inbox and reply with grounded answers.

You can also deploy Budibase Agents into external chat channels such as Discord and Teams through the agent's `Deployment` tab. That gives users a familiar interface while keeping the retrieval logic and prompt inside Budibase.

<figure>
  <img
    src={deploymentScreenshot}
    alt="Budibase agent deployment screen showing Discord and Microsoft Teams messaging channel options alongside a chat preview panel."
  />
  <figcaption>The deployment panel supports both messaging-channel rollouts and a native Budibase chat experience.</figcaption>
</figure>

Finally, Budibase provides a native authenticated chat UI. If you want a direct internal assistant experience without routing through another channel, the built-in chat interface is the simplest way to expose the agent to end users.

The common requirement across all three options is the same: make sure the retrieval tools, page mappings, and response rules are in place before you expose the assistant. The delivery channel can change, but the grounding pattern should remain identical.
